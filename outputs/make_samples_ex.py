...
def unpack(path, oracle_path):
    ...
    for index, entry in enumerate(hyp):
        with open(os.path.join(path, 'hyp', str(index)+'.txt'),'w') as f:
            f.write(entry.lower())
        found = False
        for entry_o, abs_entry_o in zip(ref, abs_ref_o):
            o_abs =  abs_entry_o.lower().replace(' ','').replace('\n','')
            c_abs = abs_ref[index].lower().replace(' ','').replace('\n','')
            if o_abs == c_abs:
              found = True
              with open(os.path.join(path, 'ref', str(index)+'.txt'),'w') as f:
                   f.write(entry_o.lower())
        if not found:
...
def rouge(path):
    ...
    with open(r.model_dir+'/'+max_id+'.txt','r') as f:
            best_ref = f.read()
    with open(path+'/worst_hyp_ex.txt','w') as f:
            f.write(worst_hyp)
    with open(path+'/worst_ref_ex.txt','w') as f:
            f.write(worst_ref)
    with open(path+'/best_hyp_ex.txt','w') as f:
            f.write(best_hyp)
    with open(path+'/best_ref_ex.txt','w') as f:
            f.write(best_ref)
